...
//| echo: false
//| output: false
test_table = aq.from(inspection_history_table)
  .fold(inspection_history_table.columnNames().slice(4, 17), {
    as: ['Question', 'Grade']
  })
  .derive({
  ...
  .groupby("CSNumber", "ServiceName", "Publication_of_Latest_Grading", "Last_inspection_Date", "URL","Grade")
  ...
  Themes: d => op.compact(op.values(op.row_object()))
   ...
   .derive({
   // replace the first 6 columns worth of data so only left with concatenated themes
    Themes: d => op.replace(d.Themes, /(.+?,){6}/, ' ')
  })
```

`{ojs} Inputs.table(selected_likert_table)`

`{ojs} Inputs.table(test_table)`