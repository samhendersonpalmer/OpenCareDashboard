...
selected_likert_table = inspection_history_table
  .fold(inspection_history_table.columnNames().slice(5, 17), {
    as: ['Question', 'Grade']
  })
  .derive({
  ...
  .derive({
  date_test: aq.escape(d => op.parse_date(d.Last_inspection_Date))})
  .derive({
  d3date_test: aq.escape(d => parser(d.Last_inspection_Date))})
  .derive({
  month_test: (d) => op.utcmonth(d.Last_inspection_Date)})

```

`{ojs} Inputs.table(selected_likert_table)`


`{ojs} Inputs.table(calc_selected_likert_table)`
...
Plot.plot({
  marks: [
    Plot.barY(calc_selected_likert_table, {
      x: "Last_inspection_Date",
      y: "proportion",
      fill: "Grade_descr",
      order: "Grade_descr",  // keeps stacking consistent
      reverse: true          // match ggplot reverse stacking
    }),
    Plot.axisY({ tickFormat: d => `${Math.round(d)}%` }),
  ...
  // style applied globally to plot
  style: {
    fontSize: "14px", 
  },
  width: 800,
...
Plot.plot({
  ...
  color: {scheme: "BuRd"},
  marks: [
    Plot.barX(calc_selected_likert_table, {
      x: "Last_inspection_Date",
      fill: "Grade_descr",
