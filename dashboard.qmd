  ...
  mutate(
    ...
    staff_per_place = ifelse(
      Registered_Places == 0 |
        NumberStaff == 0,
      NA,
      round(NumberStaff / Registered_Places, 1)
    )
  ) |> 
...
staff_check = {
  if(service_selected.staff_per_place >=0) {
    return (service_selected.staff_per_place + " staff per registered place");
  } else {
...
median_staff_subtitle  = {
  if (service_selected.CareService) {
    return "The average (median) number of staff per registered place for services of this type is " + median_staff;
  } else{
    return "";
  }
}
...
`{ojs} complaints_undefined_check`
